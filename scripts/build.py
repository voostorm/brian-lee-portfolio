...
EVIDENCE = json.loads((ROOT/'assets/evidence/manifest.json').read_text())
...
<meta property="og:image" content="{BASE}assets/evidence/fish-fish.png">
<link rel="icon" href="{prefix}assets/favicon.svg" type="image/svg+xml">
<link rel="stylesheet" href="{prefix}assets/style.css?v=20260921-evidence"><script src="{prefix}assets/site.js" defer></script></head><body>
...
def project_visual(p, prefix='', compact=False):
    item = EVIDENCE[p['id']]
    if item['kind'] == 'image':
        return f'<img src="{prefix}assets/evidence/{item["image"]}" alt="{e(item["alt"])}" loading="lazy">'
    if item['kind'] == 'document':
        return f'<div class="document-preview"><span class="file-label">{e(item["filename"])}</span><h3>AABB bounding volumes &amp; view-frustum culling</h3><p>{e(item["text"])}</p><span class="document-note">From the public milestone outline</span></div>'
    lines = item['text'].splitlines()
    if compact:
        lines = lines[:7]
    rendered = ''.join(f'<span class="code-line"><span class="line-number" aria-hidden="true">{item["start"]+n}</span><span>{e(line) or " "}</span></span>' for n,line in enumerate(lines))
    return f'<div class="source-preview"><div class="file-label">{e(item["filename"])}<span>{e(item["language"])}</span></div><pre aria-label="Source excerpt from {e(item["filename"])}"><code>{rendered}</code></pre></div>'
...
for p in PROJECTS:
    ...
    <div class="case-intro"><div><p class="eyebrow">{e(p['category'])} / {e(p['year'])}</p><h1>{e(p['title'])}</h1><p class="lead">{e(p['summary'])}</p>{tags(p)}<p class="project-context">{e(p['context'])}</p></div><figure class="case-art evidence-{evidence['kind']}">{project_visual(p, "../")}<figcaption>{e(evidence['label'])} · {link(evidence['source'], "View original")}</figcaption></figure></div>
...
for n,p in enumerate(PROJECTS,1):
    ...
    cards.append(f'''<article class="project-card" data-categories="{p['filters']}" data-search="{e(search)}"><a class="card-visual evidence-{evidence['kind']}" href="projects/{p['id']}.html" tabindex="-1" aria-hidden="true"><span class="card-number">{n:02d}</span>{project_visual(p, compact=True)}<span class="art-caption">{e(evidence['label'])}</span><span class="card-arrow">↗</span></a><div class="card-body"><p class="eyebrow">{e(p['category'])}</p><h3><a href="projects/{p['id']}.html">{e(p['title'])}</a></h3><p class="summary">{e(p['summary'])}</p>{tags(p)}<p class="card-meta">{e(p['year'])} <span aria-hidden="true">/</span> {'Team project' if 'team' in p['context'].lower() else 'Personal project' if p['id'] in ['zap','addon-engineering'] else 'Coursework'}</p></div></article>''')
...
index+='''<main id="main"><div id="top"></div><section class="hero wrap" aria-labelledby="hero-title"><div class="hero-copy"><p class="eyebrow"><span class="status-dot"></span> Open to early-career engineering roles</p><h1 id="hero-title">Built to solve.<br>Made to <em>explore.</em></h1><p class="hero-description">I’m Brian, a software engineer in Los Angeles.<br>From backend services to game systems, I turn complex problems into software you can use.</p><div class="hero-actions"><a class="button" href="#work">Explore my work <span aria-hidden="true">↓</span></a><a class="text-link" href="https://github.com/voostorm" target="_blank" rel="noopener noreferrer">GitHub ↗</a></div><div class="hero-credentials"><span>M.S. Computer Science <strong>USC ’25</strong></span><span>Backend · Systems · Games · AI</span></div></div><figure class="hero-project"><div class="feature-label"><span class="status-dot"></span> From my projects <span>01 / Fish Fish</span></div><a class="hero-capture" href="projects/fish-fish.html" aria-label="Explore Fish Fish"><img src="assets/evidence/fish-fish.png" width="960" height="600" alt="Fish Fish, captured from the playable Unity build" fetchpriority="high"></a><figcaption><div><p class="eyebrow">Unity · C# · Team project</p><h2>Fish Fish</h2><p>A fishing-combat roguelike.<br>Cast, hook, reel—and keep moving.</p></div><a class="feature-link" href="https://voostorm.github.io/fish-fish-playable/" target="_blank" rel="noopener noreferrer">Play the game ↗</a></figcaption><a class="feature-secondary" href="projects/rag-workflows.html"><img src="assets/evidence/rag-workflows.png" alt="Submitted LangFlow workflow" width="90" height="60"><span><small>ALSO EXPLORING</small>Retrieval &amp; local AI workflows</span><span aria-hidden="true">↗</span></a></figure></section>
...
<section class="about-section" id="about"><div class="wrap about-grid"><div class="about-heading"><p class="eyebrow">02 / A little about me</p><h2>Curiosity is<br>the common thread<span class="accent">.</span></h2><div class="portrait"><img src="brian_face.jpeg?v=20260921-evidence" alt="Brian Lee" width="112" height="112" loading="lazy"><div><strong>Brian (Won S.) Lee</strong><span>Software engineer · Los Angeles</span></div></div></div><div class="about-copy"><p class="lead">I like understanding how things work—and making them work better.</p><p>My projects span academic backend services, search and AI experiments, game development, and modifications to the tools I use. I enjoy the point where a user-facing problem becomes a concrete question about code, data, or system behavior.</p><p>I’m looking for an early-career engineering role where I can contribute, learn from thoughtful teammates, and keep building. I’m open to opportunities across the United States.</p><div class="education"><div><span class="edu-year">2025</span><div><h3>University of Southern California</h3><p>M.S. in Computer Science · December 2025</p></div></div><div><span class="edu-year">2023</span><div><h3>Tennessee Technological University</h3><p>B.S. in Computer Science · Cum Laude · December 2023</p></div></div></div><div class="archive-links">'''+link('https://github.com/voostorm/ttu-works','Undergraduate work')+link('https://github.com/voostorm/usc-works','Graduate work')+'''</div></div></div></section>
